...
class SnapshotTimePointSelectionConfigModel(InputsBaseModel):
    ...
    mode: SnapshotTimePointSelectionMode = Field(
        title="mode",
        description="Mode",
    ...
    search_duration_min: float = Field(
...
class ScenarioModel(InputsBaseModel):
    ...
    post_process_infos: List[ScenarioPostProcessModel] = Field(
...
class ProjectModel(InputsBaseModel):
    ...
    active_project_path: Path = Field(
    ...
    convergence_error_percent_threshold: float = Field(
...
class ExportsModel(InputsBaseModel):
    ...
    export_node_names_by_type: bool = Field(
...
class FrequencyModel(InputsBaseModel):
    ...
    enable_frequency_sweep: bool = Field(
...
class PlotsModel(InputsBaseModel):
    ...
    create_dynamic_plots: bool = Field(
...
class ReportBaseModel(InputsBaseModel):
    ...
    store_all_time_points: bool = Field(
...
class VoltageMetricsReportModel(ReportBaseModel):
    ...
    range_a_limits: List[float] = Field(
        title="range_a_limits",
        description="ANSI Range A voltage limits",
        default=[0.95, 1.05],
    )
    range_b_limits: List[float] = Field(
...
class SimulationSettingsModel(InputsBaseModel):
    ...
    monte_carlo: MonteCarloModel = Field(
        title="monte_carlo",
        description="Monte Carlo settings",
        default=MonteCarloModel(),
        alias="MonteCarlo",
    )
    plots: PlotsModel = Field(
        title="plots",
        description="Plots settings",
        default=PlotsModel(),
        alias="Plots",
    )
    profiles: ProfilesModel = Field(
        title="profiles",
        description="Profiles settings",
        default=ProfilesModel(),
        alias="Profiles",
    )
    reports: ReportsModel = Field(
        title="reports",
